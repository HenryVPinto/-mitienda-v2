...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  // Quitar el vendor actual del producto antes de asignar el nuevo
  const { data: products } = await query.graph({
    entity: "product",
    fields: ["id", "mt_vendor.*"],
    filters: { id },
  })
  const currentVendor = (products[0] as any)?.mt_vendor
  if (currentVendor?.id) {
    await remoteLink.dismiss({
      [Modules.PRODUCT]: { product_id: id },
      [VENDOR_MODULE]: { mt_vendor_id: currentVendor.id },
    }).catch(() => {})
  }
