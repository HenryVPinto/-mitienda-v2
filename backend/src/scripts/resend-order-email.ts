...
// Uso: medusa exec src/scripts/resend-order-email.ts -- --display_id=30
export default async function resendOrderEmail({ container }: ExecArgs) {
  const displayId = Number(process.argv.find((a) => a.startsWith("--display_id="))?.split("=")[1] ?? "30")
  ...
  if (!order) {
    console.error(`❌ No se encontró la orden #${displayId}`)
    return
  }

  console.log(`📦 Reenviando email para orden #${order.display_id} (${order.email}) — Total: ${formatPrice(order.total)}`)
  ...
  console.log(`✅ Email reenviado correctamente a ${order.email}`)
}
